...
class MovieListViewController: UITableViewController ,UIViewControllerPreviewingDelegate {
    ...
    var movieListDataModel:MovieDataViewModel = MovieDataViewModel()
    var currPage = 1
    ...
    private func bindToModel() {
        ...
        movieListDataModel.movies.bind { [unowned self]value in
            self.hideDataLoadingProgressHUD()
            self.tableView.reloadData()
        }
        
        // Set View Model Event Listener
        movieListDataModel.error.bind { [unowned self]error in
             self.hideDataLoadingProgressHUD()
...
extension MovieListViewController:SearchViewControllerProtocol {
    func searchCriteriaSelected(title:String,type:String?,year:String?) {
        ...
        movieListDataModel.fetchMovieListFromServerForTitle(title, type: type, year: year,atPage: currPage)
...
extension MovieListViewController {
    override func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath)
    {
        ...
        if fetchNextPage == true {
           ...
           movieListDataModel.fetchMovieListFromServerForTitle(movieTitle, type: type, year: year,atPage: currPage)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let detailVC = self.movieDetailViewControllerFor(indexPath: indexPath)
...
extension MovieListViewController {
        ...
        fileprivate func movieDetailViewControllerFor(indexPath:IndexPath)-> MovieDetailViewController {
